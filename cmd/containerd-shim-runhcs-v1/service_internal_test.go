...
func verifyExpectedStats(t *testing.T, isWCOW, ownsHost bool, s *stats.Statistics) {
	if s.GetContainer().Size() == 0 {
		t.Fatalf("container stats not implemented should be zero size got: %d", s.GetContainer().Size())
	}
	if isWCOW {
...
func verifyExpectedCgroupMetrics(t *testing.T, v *v1.Metrics) {
	t.Fatalf("cgroup metrics verification not implemented yet")
}
