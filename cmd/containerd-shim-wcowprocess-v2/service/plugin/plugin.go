...
// etwProviderName is the ETW provider name for the process shim.
const etwProviderName = "Microsoft.Virtualization.containerd-shim-wcowprocess-v2"

// svc holds the single Service instance created during plugin initialization.
var svc *service.Service

func init() {
	// Provider ID: FFCE2143-6527-59AA-7B94-823CD316B1D2
...
// etwCallback dumps host goroutine stacks on a CaptureState notification.
func etwCallback(sourceID guid.GUID, state etw.ProviderState, level etw.Level, matchAnyKeyword uint64, matchAllKeyword uint64, filterData uintptr) {
	if state == etw.ProviderStateCaptureState {
		...
		logrus.WithField(logfields.SandboxID, svc.SandboxID()).WithField("stack", resp.Stacks).Info("goroutine stack dump")
	}
}
