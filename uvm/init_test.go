// +build functional scsi create sandbox
...
func createLCOWUVM(t *testing.T, id string) *UtilityVM {
	opts := &UVMOptions{OperatingSystem: "linux"}
	if id != "" {
		opts.ID = id
	}
	uvm, err := Create(opts)
...
func createLCOWTempDirWithSandbox(t *testing.T, vmID string) string {
	...
	tempDir := createTempDir(t)
	if err := lcowGlobalSVM.CreateLCOWSandbox(filepath.Join(tempDir, "sandbox.vhdx"), DefaultLCOWSandboxSizeGB, lcowCacheSandboxFile, vmID); err != nil {
