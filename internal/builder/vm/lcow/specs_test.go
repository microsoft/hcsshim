...
// io.microsoft.migration.allowed sandbox annotation. The annotation is parsed
// into SandboxOptions.LiveMigrationAllowed and threaded down into the kernel
	...
	tests := []specTestCase{
		{
			...
			validate: func(t *testing.T, doc *hcsschema.ComputeSystem, sandboxOpts *SandboxOptions) {
				...
				if sandboxOpts.LiveMigrationAllowed {
					t.Errorf("expected LiveMigrationAllowed=false by default, got true")
				}
				kernelArgs := getKernelArgs(doc)
		...
		},
		{
			...
			spec: &vm.Spec{
				Annotations: map[string]string{
					shimannotations.LiveMigrationAllowed: "false",
				},
			},
			validate: func(t *testing.T, doc *hcsschema.ComputeSystem, sandboxOpts *SandboxOptions) {
				t.Helper()
				if sandboxOpts.LiveMigrationAllowed {
					t.Errorf("expected LiveMigrationAllowed=false when annotation=\"false\", got true")
				}
				kernelArgs := getKernelArgs(doc)
				if !strings.Contains(kernelArgs, vsockexecPrefix) {
		...
		},
		{
			...
			validate: func(t *testing.T, doc *hcsschema.ComputeSystem, sandboxOpts *SandboxOptions) {
				t.Helper()
				if !sandboxOpts.LiveMigrationAllowed {
					t.Errorf("expected LiveMigrationAllowed=true when annotation=\"true\", got false")
				}
				kernelArgs := getKernelArgs(doc)
		...
		},
		{
			...
			validate: func(t *testing.T, doc *hcsschema.ComputeSystem, sandboxOpts *SandboxOptions) {
				t.Helper()
				if !sandboxOpts.LiveMigrationAllowed {
					t.Errorf("expected LiveMigrationAllowed=true, got false")
				}
				kernelArgs := getKernelArgs(doc)
		...
		},
		{
			...
			spec: &vm.Spec{
				Annotations: map[string]string{
					...
					shimannotations.LiveMigrationAllowed: "not-a-bool",
				},
			},
			validate: func(t *testing.T, doc *hcsschema.ComputeSystem, sandboxOpts *SandboxOptions) {
				t.Helper()
				if sandboxOpts.LiveMigrationAllowed {
					t.Errorf("expected LiveMigrationAllowed=false on invalid annotation value, got true")
				}
				kernelArgs := getKernelArgs(doc)
				if !strings.Contains(kernelArgs, vsockexecPrefix) {
					t.Errorf("expected vsockexec wrapper %q in kernel args, got %q", vsockexecPrefix, kernelArgs)
