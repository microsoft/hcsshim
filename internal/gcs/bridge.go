...
type bridge struct {
	...
	waitCh  chan struct{}
	brdgErr error // read after <-waitCh

	mu sync.Mutex
	// protected by mu:
	closed bool
	nextID int64
	rpcs   map[int64]*rpc // todo: replace with sync.Map
}
...
func (brdg *bridge) writeMessage(buf *bytes.Buffer, enc *json.Encoder, mi prot.MessageIdentifier, id int64, req interface{}) (err error) {
	// Save room for header in the buffer
	h := make([]byte, prot.MessageHeaderSize)
	buf.Write(h)

	if err = enc.Encode(req); err != nil {
		return fmt.Errorf("bridge request encode: %w", err)
	}
