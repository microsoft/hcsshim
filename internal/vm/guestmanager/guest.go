...
func New(ctx context.Context, uvm interface {
	...
	connID := winio.VsockServiceID(prot.LinuxGcsVsockPort)
	conn, err := uvm.VMSocketListen(connID)
	if err != nil {
		return nil, errors.Wrap(err, "failed to listen for guest connection")
	}
	gm.gcListener = conn
...
func (gm *Guest) CreateConnection(ctx context.Context, opts ...ConfigOption) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to connect to GCS")
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to create GCS connection")
	}
