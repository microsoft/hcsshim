...
func (uvm *UtilityVM) deallocateVPMEM(ctx context.Context, deviceNumber uint32) error {
	uvm.m.Lock()
	defer uvm.m.Unlock()
...
func (uvm *UtilityVM) AddVPMEM(ctx context.Context, hostPath string, expose bool) (_ uint32, _ string, err error) {
	...
	log.G(ctx).WithFields(logrus.Fields{
		...
		"uvm-path":      uvmPath,
		"deviceNumber":  deviceNumber,
	}).Debug("hcsshim::AddVPMEM add at location")
...
func (uvm *UtilityVM) RemoveVPMEM(ctx context.Context, hostPath string) (err error) {
	ctx, span := trace.StartSpan(ctx, "uvm::RemoveVPMEM")
	defer span.End()
	defer func() { oc.SetSpanStatus(span, err) }()
	span.AddAttributes(
		trace.StringAttribute(logfields.UVMID, uvm.id),
