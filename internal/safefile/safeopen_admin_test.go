...
func TestOpenRelative(t *testing.T) {
	...
	// Create a file
	f, err := OpenRelative("foo", root, 0, syscall.FILE_SHARE_READ, ntconstants.FILE_CREATE, 0)
	if err != nil {
		t.Fatal(err)
	}
	f.Close()
	...
	f, err = OpenRelative("dsymlink/foo", root, 0, syscall.FILE_SHARE_READ, ntconstants.FILE_CREATE, 0)
	if err == nil {
		f.Close()
	...
	f, err = OpenRelative("..", root, syscall.GENERIC_READ, syscall.FILE_SHARE_READ, ntconstants.FILE_OPEN, 0)
	if err == nil {
