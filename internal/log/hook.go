...
var _bytesBufferType = reflect.TypeOf(bytes.Buffer{}).String()
...
func (h *Hook) encode(e *logrus.Entry) {
	...
	for k, v := range d {
		...
		// if  `err != nil`, then `b == nil` and this will be the the empty string
		d[k] = string(b)
	}
}
...
// byteArrayValueToSlice creates a slice from the underlying data in the [n]byte array
// rv *MUST* be of type byte array
func byteArrayValueToSlice(rv reflect.Value) []byte {
	l := rv.Len()
	b := make([]byte, l)
	for i := 0; i < rv.Len(); i++ {
