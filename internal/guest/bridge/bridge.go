	...
	go func() {
		...
		for {
			if atomic.LoadUint32(&b.hasQuitPending) == 0 {
				...
				if base.SpanContext.IsValid() {
					ctx = trace.ContextWithRemoteSpanContext(ctx, base.SpanContext)
				}

				ctx, _ = otelutil.StartSpan(ctx, "opengcs::bridge::request", otelutil.WithServerSpanKind, trace.WithAttributes(
...
func (b *Bridge) PublishNotification(n *prot.ContainerNotification) {
	ctx, span := otelutil.StartSpan(context.Background(),
		"opengcs::bridge::PublishNotification",
		otelutil.WithClientSpanKind,
		trace.WithAttributes(attribute.String("notification", fmt.Sprintf("%+v", n))))
