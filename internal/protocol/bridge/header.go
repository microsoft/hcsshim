...
const (
	...
	mhTypeOffset = unsafe.Offsetof(MessageHeader{}.Type)
	mhSizeOffset = unsafe.Offsetof(MessageHeader{}.Size)
	mhIDOffset   = unsafe.Offsetof(MessageHeader{}.ID)
)
...
func (h *MessageHeader) Unmarshal(b []byte) (err error) {
	...
	h.Type = MessageIdentifier(binary.LittleEndian.Uint32(b[mhTypeOffset:]))
	h.Size = binary.LittleEndian.Uint32(b[mhSizeOffset:])
	h.ID = SequenceID(binary.LittleEndian.Uint64(b[mhIDOffset:]))
...
func (h *MessageHeader) MarshalTo(b []byte) (int, error) {
	if len(b) < MessageHeaderSize {
		return 0, fmt.Errorf("cannot write message header from buffer: %w", io.ErrShortBuffer)
	}
	b = b[:MessageHeaderSize]

	binary.LittleEndian.PutUint32(b[mhTypeOffset:], uint32(h.Type))
	binary.LittleEndian.PutUint64(b[mhSizeOffset:], uint64(h.Size))
	binary.LittleEndian.PutUint32(b[mhIDOffset:], uint32(h.ID))

	return MessageHeaderSize, nil
}
