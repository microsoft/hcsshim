...
func enablePrivilege(privName string) error {
	...
	defer func() {
		if derr := token.Close(); derr != nil {
			logrus.WithError(derr).Warn("Failed to close token")
		}
	}()

	var luid windows.LUID
	namePtr, nerr := windows.UTF16PtrFromString(privName)
	if nerr != nil {
		logrus.WithError(nerr).Errorf("invalid privilege name %q", privName)
		return nerr
	}

	if err := windows.LookupPrivilegeValue(nil, namePtr, &luid); err != nil {
		logrus.WithError(err).Error("LookupPrivilegeValue failed")
		return err
	}
	...
	lastErr := error(windows.GetLastError())
	if errors.Is(lastErr, windows.ERROR_SUCCESS) {
		logrus.Info("Privilege enabled successfully.")
	} else {
		logrus.WithError(lastErr).Error("AdjustTokenPrivileges warning")
	}
...
func VsmbMain() {
	...
	if err := enablePrivilege(SeLoadDriverName); err != nil {
	...
	status := winapi.NtFsControlFile(
		...
		buffer,                // input buffer
		uint32(len(buffer)),   // input length
		nil,                   // output buffer
		0,                     // output length
	)
	switch status {
	...
	status = winapi.NtFsControlFile(
		...
		bindBuffer,              // in
		uint32(len(bindBuffer)), // inLength
		nil,                     // out
		0,                       // outLength
	)
	if status == 0 {
