...
// expectVMRunsUntilCleanup mirrors a live VM for the background goroutine that
// StartVM launches (waitForVMExit): uvm.Wait blocks until the test finishes and
// the post-exit CloseConnection the goroutine performs is permitted. Without it,
// the mock Wait would return immediately, letting the goroutine transition the
// controller to Terminated mid-assertion or call into the mock after the test
// has completed (which gomock reports as an unexpected call).
func expectVMRunsUntilCleanup(t *testing.T, uvm *mocks.MockutilityVM, guest *mocks.MockguestManager) {
	t.Helper()
...
func TestStartVM(t *testing.T) {
	...
	t.Run("create_connection_fails/state_invalid", func(t *testing.T) {
		...
		guest.EXPECT().CreateConnection(gomock.Any(), gomock.Any()).Return(errors.New("create conn failed"))
	...
	t.Run("full_success/state_running", func(t *testing.T) {
		...
		// race with the state assertion below.
		expectVMRunsUntilCleanup(t, uvm, guest)
...
func TestWaitForVMExit(t *testing.T) {
	...
	t.Run("natural_exit/sets_terminated", func(t *testing.T) {
		...
		// On VM exit, waitForVMExit collapses the bridge to release in-flight
		// waits; CloseConnection is idempotent.
		guest.EXPECT().CloseConnection().Return(nil)
	...
	t.Run("concurrent_terminate/waitForVMExit_noops", func(t *testing.T) {
		...
		uvm.EXPECT().Terminate(gomock.Any()).Return(nil)
		// Both TerminateVM and the unblocked waitForVMExit call CloseConnection
		// (idempotent); allow either ordering and count.
		guest.EXPECT().CloseConnection().Return(nil).AnyTimes()
		uvm.EXPECT().Close(gomock.Any()).Return(nil)
