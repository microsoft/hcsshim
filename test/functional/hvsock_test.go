	...
	for _, tc := range hvsockHostBindTestCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.hvsockConfig != nil {
				t.Logf("adding HVSocket config setting to WCOW uVM options for service %v: %#+v", svcGUID, tc.hvsockConfig)
				if opts.AdditionalHyperVConfig == nil {
					opts.AdditionalHyperVConfig = map[string]hcsschema.HvSocketServiceConfig{}
				}
				opts.AdditionalHyperVConfig[svcGUID.String()] = *tc.hvsockConfig
			}
...
var hvsockServiceGUID = sync.OnceValues(func() (guid.GUID, error) {
