...
func TestScratchCreateLCOW(t *testing.T) {
	...
	firstUVM := testutilities.CreateLCOWUVM(ctx, t, "TestCreateLCOWScratch")
	defer firstUVM.Close(ctx)
	...
	targetUVM := testutilities.CreateLCOWUVM(ctx, t, "TestCreateLCOWScratch_target")
	defer targetUVM.Close(ctx)
	...
	c, l, err := targetUVM.AddSCSI(ctx, destTwo, "", false)
	if err != nil {
		t.Fatal(err)
	}
	if c != 0 && l != 0 {
