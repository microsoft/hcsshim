...
func Test_RunPodSandbox_KernelOptions_LCOW(t *testing.T) {
	...
	hugePagesCmd := []string{"grep", "-i", "huge", "/proc/meminfo"}
	...
	if !strings.Contains(string(output), "HugePages_Total:      10") {
		t.Fatalf("Expected number of hugepages to be 10. Got output instead: %s", string(output))
	}
}
