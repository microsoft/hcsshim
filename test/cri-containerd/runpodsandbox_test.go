...
func Test_RunPodSandbox_MultipleContainersSameVhd_LCOW(t *testing.T) {
	...
	currDir, _ := os.Getwd()
	ext4Vhd := fmt.Sprintf("vhd://%s/TestVhds/%s", currDir, "Test-ext4.vhdx")
	destDirectory := "/containerDir"

	mounts := []*runtime.Mount{
		{
			HostPath:      ext4Vhd,
			ContainerPath: destDirectory,
		},
	}
...
func Test_RunPodSandbox_MultipleContainersSameVhd_WCOW_Hypervisor(t *testing.T) {
	pullRequiredImages(t, []string{imageWindowsNanoserver})
	...
	currDir, _ := os.Getwd()
	ntfsVhd := fmt.Sprintf("vhd://%s/TestVhds/%s", currDir, "Test.vhdx")
	destDirectory := "c:\\containerDir"
	mounts := []*runtime.Mount{
		{
			HostPath:      ntfsVhd,
			ContainerPath: destDirectory,
	...
	for i := 1; i < 3; i++ {
		...
		// For container 1 and 2 we should
		if exitCode != 0 {
			t.Fatalf("Exec into container failed with: %v and exit code: %d, %s", errorMsg, exitCode, containerId)
		}
	}
