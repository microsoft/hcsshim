...
Additionally, the shim (and other Windows binaries) require linking with a Windows manifest
(resource object) to properly function (See: `scripts\New-ResourceObjectFile.ps1` for more information).
They can be created by running `go generate` before building.
...
Alternatively, [CMake](#cmake) can be used to handle building:

```powershell
> cmake --preset "x64-release"
> cmake --build .\out\x64-release\build\ --target shim 
```
...
```powershell
> ctr.exe run --runtime io.containerd.runhcs.v1 --rm mcr.microsoft.com/windows/nanoserver:2004 windows-test cmd /c "echo Hello World!"
```
...
```powershell
> $env:GOOS="windows"
> go build -tags lcow .\cmd\containerd-shim-lcow-v2
```
...
This repo uses the [CMake (meta-)build system][cmake-doc] for building the various binaries
within the repo, as well as other automating other tooling and management tasks.

The [CMake VSCode extension][cmake-ext], along side the docs, provides an easy way to onboard
to using it.
...
[cmake-ext]: aka.ms/vscode-cmake-tools
